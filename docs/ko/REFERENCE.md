...
| **Token Tracking**      | 세션 토큰 로깅 및 런타임 추세            | `.omc/state/token-tracking.jsonl` |
...
```bash
omc                    # OMC 런타임 실행
omc hud                # 현재 HUD 상태줄 출력
# 에이전트/세션 관측: HUD + replay 로그 사용
# 토큰 추적 로그: .omc/state/token-tracking.jsonl
```

### HUD 프리셋

상태 표시줄에서 HUD 관측 정보를 활성화합니다:
...
사용 가능한 프리셋: `minimal`, `focused`, `full`, `dense`, `opencode`
