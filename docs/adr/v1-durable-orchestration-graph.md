...
atomic create whose owner JSON is `{ version: 2, pid, createdAt, expires_at,
nonce }` (the **lease schema**, `version: 2`). `expires_at` is the **sole
...
- **v2 (lease schema, current):** `{ version: 2, pid, createdAt, expires_at,
  nonce }`. The only schema evaluated for lease expiry/corrupt reclaim. New
  owners always write `version: 2`.
- **v1 (old liveness schema, pre-upgrade):** `{ version: 1, pid, processStart,
  createdAt, nonce }` - has `processStart`, no `expires_at`. This is a lock held
  under the OLD liveness contract; the owner may still be alive. It is **NOT
  corrupt**. `readLockOwner` returns a distinct `'old_version'` status for it,
  and `acquireLockAt` **fails closed** on that status (logs
  `state_mutation_lock_old_version`, returns null, never unlinks, never
  reclaims). The `flock` path signals the same via `LOCK_REMOVAL_SCRIPT` exit
  code 5 (`guardedLockRemoval` maps it to `'old_version'`). This fixes the
  blocker where a pre-upgrade owner's lock (same `version: 1`, different shape)
  was classified 'corrupt' and unlinked under the live owner - stealing a live
  lock. A version-1 record that carries `expires_at` but no `processStart` (a
  shape that never existed in production) is neither v2-lease nor v1-liveness and
  is classified `'corrupt'` (reclaimed), since no valid lease is on record.
...
1. **Holder re-validation before publish (`assertMutationLockHeld`).** Immediately
   before publishing (in `withStateFileMutationLock`/`withStateFileLockSync`
   before the callback, and in `writeStateFileLocked`/
   `writeStateFileLockedIf`/`writeStateFileLockedCreateIf`/
   `clearStateFileLocked`/`clearStateFileLockedIf` right before the
   `atomicWriteJsonSync`/`unlinkSync`), the holder re-reads its own lock's
   `expires_at` and checks `now < expires_at` and that the on-disk owner is still
   itself. If the lease has expired (or the on-disk owner was replaced by a later
   writer), it ABORTS the publish, throws `lease_expired_during_mutation`, and
   logs `state_mutation_lock_lease_expired_during_mutation`. The two-writer
   overlap is thus observable, not silent. The holder checks its OWN lease, not
   someone else's.
2. **Lease renewal for long-held locks (`renewMutationLock`).** Callers with long
   critical sections (e.g. a graph store mutation that may approach or exceed
   `LEASE_MS`) should call `renewMutationLock(lock)` periodically to extend
   `expires_at = now + LEASE_MS` while still holding the lock. Renewal
...
a valid unexpired v2 lease is LIVE (do not steal; wait/fail closed); a valid
expired v2 lease is RECLAIMED (`unlinkSync` + re-acquire); a readable but
...
FAILS CLOSED (B12: the pre-upgrade owner may be alive; never reclaim); an I/O
...
changed incompatibly while version stayed 1, a pre-upgrade lock stolen as
'corrupt') - the version is bumped to 2 and v1 old-liveness records fail closed
as `'old_version'`, never reclaimed.
