...
import { acquireMutationLockAt, releaseMutationLockSync, renewMutationLock, validateMutationLockOwner, } from '../mode-state-io.js';
 ...
 */
function freshLeaseOwner(pid, createdAt, expiresAt) {
    const now = Date.now();
    return {
        version: 1,
        pid,
...
describe('lease-based mutation lock: validateMutationLockOwner', () => {
    ...
    it('rejects an owner with extra keys, wrong types, bad nonce, or missing expires_at', () => {
        ...
        expect(validateMutationLockOwner({ version: 1, pid: 1, createdAt: new Date().toISOString(), nonce: randomUUID() })).toBeNull();
        ...
        expect(validateMutationLockOwner({ version: 3, pid: 1, createdAt: new Date().toISOString(), expires_at: new Date(Date.now() + 1000).toISOString(), nonce: randomUUID() })).toBeNull();
        expect(validateMutationLockOwner({ version: 2, pid: 1, createdAt: new Date().toISOString(), expires_at: new Date(Date.now() + 1000).toISOString(), nonce: randomUUID(), extra: 1 })).toBeNull();
    });
});
...
describe('lease-based mutation lock: renewal', () => {
    ...
    it('atomically replaces a still-live owner record while preserving ownership', () => {
        ...
        const before = JSON.parse(readFileSync(`${filePath}.mutation.lock`, 'utf8'));
        expect(before.version).toBe(1);
        const flockAvailable = existsSync('/usr/bin/flock') || existsSync('/bin/flock');
        expect(renewMutationLock(lock)).toBe(flockAvailable);
        const after = JSON.parse(readFileSync(`${filePath}.mutation.lock`, 'utf8'));
        if (!flockAvailable) {
...
describe('flock-less mutation lock: acquire -> contend -> release cycle (B4.2/B4.3 .mjs)', () => {
    ...
    it('A acquires, B fails while A is live (no steal), A releases, B acquires', () => {
        ...
        expect(a).not.toBeNull();
        expect(a.unlocked).not.toBe(true);
        const lockPath = `${filePath}.mutation.lock`;
        expect(existsSync(lockPath)).toBe(true);
        ...
        expect(onDisk.nonce).toBe(a.owner.nonce);
...
describe('flock-less mutation lock: steal-prevention (B4.4)', () => {
    ...
    it('reclaims a lock whose lease has EXPIRED (holder crashed / did not release)', () => {
        ...
        expect(JSON.parse(readFileSync(lockPath, 'utf8')).nonce).toBe(a.owner.nonce);
        releaseStateFileLockSync(a);
    });
