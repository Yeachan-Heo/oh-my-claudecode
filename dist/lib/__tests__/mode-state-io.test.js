...
describe('mode-state-io', () => {
    ...
    describe('durable emergency mutation journal', () => {
        it('skips a conditional write that races a malformed canonical replacement', () => {
            ...
            expect(writeStateFileLockedIf(path, (state) => state.active === true, (state) => ({ ...state, changed: true }))).toBe('skipped');
        ...
        it('skips a conditional create that races a malformed canonical replacement', () => {
            ...
            expect(writeStateFileLockedCreateIf(path, (current) => current === null, () => ({ active: true, run: 'created' }))).toBe('skipped');
            expect(readFileSync(path, 'utf8')).toBe(replacement);
        });
        it('makes an emergency publication visible to a later conditional write', () => {
            ...
            expect(emergencyMutateStateFileIf(path, (state) => state.run === 'emergency-visible' && state.active === true, (state) => ({ ...state, active: false }))).toBe(true);
            ...
            expect(writeStateFileLockedIf(path, (state) => state.run === 'emergency-visible' && state.active === true, (state) => ({ ...state, staleOverwrite: true }))).toBe('skipped');
        ...
        it('records an emergency clear as an OCC tombstone for a later create', () => {
            ...
            const path = join(tempDir, '.omc', 'state', 'autopilot-state.json');
            expect(emergencyMutateStateFileIf(path, (state) => state.run === 'emergency-clear', null)).toBe(true);
            expect(existsSync(path)).toBe(false);
            expect(writeStateFileLockedCreateIf(path, (current) => current === null, () => ({ active: true, run: 'created-after-emergency-clear' }))).toBe('written');
            expect(JSON.parse(readFileSync(path, 'utf8'))).toMatchObject({
                active: true,
        ...
        it('retains the authenticated journal when publication crashes before its OCC fence', () => {
            ...
            expect(emergencyMutateStateFileIf(path, (state) => state.run === 'fence-retry', (state) => ({ ...state, active: false }))).toBe(false);
            delete process.env.OMC_TEST_EMERGENCY_CRASH_PHASE;
