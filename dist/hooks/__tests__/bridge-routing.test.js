...
$ ultrawork search the codebase`,
        ...
        it('does not arm ralplan state for keywords inside delegated /ask gjc prompts', async () => {
            const tempDir = mkdtempSync(join(tmpdir(), 'bridge-routing-ask-gjc-'));
            try {
                execFileSync('git', ['init'], { cwd: tempDir, stdio: 'pipe' });
                const sessionId = 'ask-gjc-session';
                const result = await processHook('keyword-detector', {
                    sessionId,
                    prompt: '/ask gjc 지금까지 논의한걸 ralplan으로 계획서 작성해줘',
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
                expect(result.message).toBeUndefined();
