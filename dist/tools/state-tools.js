...
import { isModeActive, getActiveModes, getAllModeStatuses, clearModeStateDetailed, getStateFilePath, MODE_CONFIGS, getActiveSessionsForMode, } from "../hooks/mode-registry/index.js";
...
const STATE_CLEAR_MODES = [...STATE_TOOL_MODES, "all"];
...
export function redactGraphPublicState(state) {
    try {
        const graph = parseGraphState(state);
        const activationStatuses = Object.values(graph.projection.activations).map((activation) => activation.status);
        ...
        return {
            ...
            claims: {
                ...
                reconciling: claims.filter((claim) => claim.status === "reconciling")
                    .length,
                unresolved_reconciliations: reconciliations.filter((reconciliation) => reconciliation.status === "unresolved").length,
            },
            pending: {
...
function classifyGraphRunStatus(status) {
    if (MODE_CONFIGS.graph.activeStatuses?.includes(status))
        return "active";
    if (status === "draft")
        return "draft";
    if (status === "paused")
...
function hasGraphWorkflowSlot(root, sessionId) {
    ...
    return paths.some((path) => {
        const state = readJsonRecord(path);
        return Boolean(state?.active_skills &&
            typeof state.active_skills === "object" &&
            state.active_skills.graph);
    });
}
/**
...
function clearSkillActiveStateForAll(root, sessionId) {
    const legacyCandidates = discoverStatePaths(getLegacyStateFileCandidates("skill-active", root)).filter((candidate) => !sessionId || canClearStateForSession(candidate.state, sessionId));
    const localCandidates = discoverStatePaths(getWorkingDirectoryLocalStateClearCandidates("skill-active", root, sessionId)).filter((candidate) => !sessionId || canClearStateForSession(candidate.state, sessionId));
    ...
    const candidates = [
        ...new Map([...legacyCandidates, ...localCandidates, ...sessionCandidates].map((candidate) => [candidate.path, candidate])).values(),
    ];
    let changed = false;
    let failed = false;
    ...
    for (const candidate of candidates) {
        ...
        const graphIsPresent = skills !== null &&
            typeof skills === "object" &&
            !Array.isArray(skills) &&
            Object.prototype.hasOwnProperty.call(skills, "graph");
        if (!graphIsPresent) {
            const result = clearDiscoveredStateCandidate(candidate, (current) => (!sessionId || canClearStateForSession(current, sessionId)) &&
                JSON.stringify(current) === candidate.snapshot);
            if (result === "cleared")
                changed = true;
            else if (result === "failed" ||
        ...
        if (nonGraphSkills.length === 0)
            continue;
        const result = writeStateFileLockedIf(candidate.path, (current) => (!sessionId || canClearStateForSession(current, sessionId)) &&
            JSON.stringify(current) === candidate.snapshot, (current) => {
...
export const stateClearTool = {
    ...
    schema: {
        ...
        force: z
            .boolean()
            .optional()
            .describe("Request broad cleanup. This never bypasses guarded Graph state."),
        workingDirectory: z
            .string()
            .optional()
            .describe("Working directory (defaults to cwd)"),
        ...
        try {
            ...
            if (mode === "all") {
                ...
                for (const candidate of STATE_TOOL_MODES) {
                    ...
                    if (candidate === "skill-active") {
                        ...
                        if (result.failed)
                            failedModes.push(candidate);
                        else if (result.changed)
                            clearedModes.push(candidate);
                        continue;
                    }
            ...
            if (sessionId) {
                ...
                if (MODE_CONFIGS[mode]) {
                    ...
                    const registryClearResult = clearModeStateDetailed(mode, root, sessionId, expectedDirectState);
                    // A CAS skip means the canonical path now belongs to a newer run. Do
                    // not feed that path into the discovery cleanups below.
                    const skippedCanonicalPaths = new Set(registryClearResult === "skipped" && directCandidate
                        ? [directCandidate.path]
                        : []);
                    const success = registryClearResult === "cleared";
                    if (directCandidate && !existsSync(directCandidate.path))
                        directCleared = 1;
                    const sessionCleanup = clearSessionOwnedStateCandidates(mode, root, sessionId, requestedSessionCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)));
                    const legacyCleanup = clearLegacyStateCandidates(mode, root, sessionId, legacyCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)));
                    ...
                    const workingDirectoryLocalCleanup = shouldUseLocalFallback
                        ? clearWorkingDirectoryLocalStateCandidates(mode, root, sessionId, localCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)))
                        : { cleared: 0, hadFailure: false, paths: [] };
                    convergedCleanup = clearConvergedStateCandidates(mode, root, sessionId, convergedCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)));
                    let ownerSessionId;
                    ...
                    if (OWNER_SESSION_FALLBACK_MODES.has(mode) &&
                        ...
                        if (ownerSessionId) {
                            ...
                            if (ownerNamedPrimary) {
                                ...
                                const ownerClearResult = clearModeStateDetailed(mode, root, ownerSessionId, ownerDirectCandidate?.state);
                                const skippedOwnerCanonicalPath = ownerClearResult === "skipped"
                                    ? ownerDirectCandidate?.path
                                    : undefined;
                                ownerSessionCleanup = clearSessionOwnedStateCandidates(mode, root, ownerSessionId, ownerCandidates.filter((candidate) => candidate.path !== skippedOwnerCanonicalPath));
            ...
            if (MODE_CONFIGS[mode]) {
                ...
                if (primaryCandidate) {
                    const clearResult = clearModeStateDetailed(mode, root, undefined, primaryCandidate.state);
                ...
                const extraLegacyCleanup = clearLegacyStateCandidates(mode, root, undefined, broadLegacyCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)));
                clearedCount += extraLegacyCleanup.cleared;
                if (extraLegacyCleanup.hadFailure) {
                    errors.push("legacy path");
                }
                const convergedCleanup = clearConvergedStateCandidates(mode, root, undefined, broadConvergedCandidates.filter((candidate) => !skippedCanonicalPaths.has(candidate.path)));
