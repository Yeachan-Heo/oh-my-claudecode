    ...
    return {
        ...
        workflow: {
            ...
            profileHash: createHash("sha256")
                .update('{"descriptorVersion":1,"profileVersion":1,"stages":["ralplan","execution"],"workflowName":"release-train"}')
                .digest("hex"),
        },
        pipelineTracking: {
            stages: [
...
function graphState(sessionId, status = "running") {
        ...
        ...(approved
            ? {
                approval: {
                    ...
                    evidence: { kind: "human", ref: "private-human-evidence" },
                },
            }
            : {}),
    });
...
function writeGuardedGraphArtifacts(sessionId, status = "running") {
    ...
    writeFileSync(paths.graph, status === "malformed"
        ? "{not-json"
...
describe("state-tools", () => {
        ...
        it("pauses only an authenticated exact named run without flock and preserves every resume field", async () => {
            ...
            const state = {
                ...
                workflow: {
                    ...descriptor,
                    profileHash: createHash("sha256")
                        .update('{"descriptorVersion":1,"profileVersion":1,"stages":["ralplan","execution"],"workflowName":"release-flow"}')
                        .digest("hex"),
                },
                pipelineTracking: {
                    stages: [
    ...
    describe("state_clear", () => {
        it.each([
            ["supported", "1"],
            ["no-flock", "0"],
        ])("clears an exact malformed marker-bearing snapshot under %s runtime", async (_runtime, flock) => {
        ...
        it.each([
            ["supported", "1"],
            ["no-flock", "0"],
        ])("preserves a replacement that races an exact malformed-marker clear under %s runtime", async (_runtime, flock) => {
        ...
        it("clears recovered malformed named primaries during session cleanup", async () => {
            ...
            writeFileSync(stranded, JSON.stringify({
                ...state,
                workflowRunId: "22222222-2222-4222-8222-222222222222",
            }));
            process.env.OMC_TEST_FLOCK_AVAILABLE = "0";
        ...
        it("clears an incomplete named state without starting a portable pause transaction", async () => {
            ...
            expect((await stateWriteTool.handler({
                mode: "autopilot",
                active: false,
                session_id: sessionId,
                state: { workflowRunId: state.workflowRunId },
        ...
        it("preserves a foreign dead publication temp beside an authorized home-global state", async () => {
            ...
            try {
                ...
                writeFileSync(foreignTemp, JSON.stringify({
                    active: false,
        ...
        it("preserves unrelated shared-session autopilot state and emergency artifacts during broad clear", async () => {
            ...
            try {
                ...
                writeFileSync(projectAPath, JSON.stringify({
                    active: true,
                    project_path: TEST_DIR,
        ...
        it("clears completed-session orphan state when cancel runs from a fresh session id", async () => {
            ...
            for (const orphanSessionId of orphanSessionIds) {
                ...
                writeFileSync(join(TEST_DIR, ".omc", "sessions", `${orphanSessionId}.json`), JSON.stringify({
        ...
        it("preserves a replacement run at a completed-session candidate path", async () => {
            ...
            mkdirSync(dirname(statePath), { recursive: true });
            mkdirSync(join(TEST_DIR, ".omc", "sessions"), { recursive: true });
            writeFileSync(join(TEST_DIR, ".omc", "sessions", `${endedSession}.json`), JSON.stringify({
        ...
        it("clears completed-session orphan state through a symlinked .omc directory", async () => {
            ...
            try {
                ...
                writeFileSync(join(realOmcDir, "sessions", `${orphanSessionId}.json`), JSON.stringify({
                    session_id: orphanSessionId,
                    ended_at: "2026-05-04T00:00:00.000Z",
                }));
    ...
    describe("state_get_status", () => {
        ...
        it("should return all mode statuses when no mode specified", async () => {
            ...
            expect(result.content[0].text.includes("[ACTIVE]") ||
                result.content[0].text.includes("[INACTIVE]")).toBe(true);
        });
    });
    describe("session_id parameter", () => {
        ...
        it("should clear session-specific state without affecting legacy owned by another session", async () => {
            ...
            writeFileSync(join(TEST_DIR, ".omc", "state", "ralph-state.json"), JSON.stringify({
                active: true,
                source: "legacy",
        ...
        it("should clear recovered session-owned state stranded under another session directory", async () => {
            ...
            writeFileSync(join(strandedDir, "ralph-state.json"), JSON.stringify({
                active: true,
                session_id: sessionId,
    ...
    describe("Graph guarded state integration", () => {
        ...
        it("returns a bounded Graph summary without descriptor, evidence, or diagnostic details", async () => {
            ...
            expect(result.content[0].text).not.toMatch(/private diagnostic detail|private-human-evidence|Build and verify two branches|run-branch-b/);
        });
        ...
        it.each([
            ...
            ["succeeded", "terminal"],
            ["malformed", "malformed"],
        ])("returns deterministic guarded clear classification for %s Graph state", async (status, classification) => {
