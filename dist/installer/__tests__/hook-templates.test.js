...
const provisionedTemplatePaths = new Map();
function provisionedTemplatePath(scriptPath) {
    if (!scriptPath.includes('/templates/hooks/'))
        return scriptPath;
...
function provisionedTemplateLibPath(helperPath) {
    if (!helperPath.includes('/templates/hooks/'))
        return helperPath;
...
function runKeywordHook(scriptPath, prompt) {
    return JSON.parse(execFileSync('node', [provisionedTemplatePath(scriptPath)], {
        cwd: packageRoot,
...
이런대화가 한번이라면 몇번할수있을까 오픈라우터 20달러 결제기준 api로`,
    ...
    it('honors keywordDetector.disabled from .claude/omc.jsonc in both packaged artifacts', () => {
        ...
        const runInDir = (scriptPath, prompt, dir) => JSON.parse(execFileSync('node', [provisionedTemplatePath(scriptPath)], {
            cwd: packageRoot,
    ...
    it('disambiguates bare ralph when the official ralph-loop plugin is installed and enabled (#3668)', () => {
        ...
        const runWithEnv = (scriptPath, sessionId, prompt, env) => JSON.parse(execFileSync('node', [provisionedTemplatePath(scriptPath)], {
            cwd: packageRoot,
            env: {
                ...process.env,
                HOME: fakeHome,
        ...
        try {
            ...
            for (const scriptPath of [templatePath, pluginPath]) {
                const result = JSON.parse(execFileSync('node', [provisionedTemplatePath(scriptPath)], {
                    cwd: packageRoot,
            ...
            try {
                ...
                const runFromSandbox = (scriptPath, sessionId, payloadCwd) => JSON.parse(execFileSync('node', [provisionedTemplatePath(scriptPath)], {
                    cwd: sandboxCwd,
                    env: {
                        ...process.env,
                        HOME: fakeHome,
    ...
    it('keeps SessionStart occupancy single-host: publish takes a precomputed identity and read reuses an identity map', async () => {
        ...
        try {
            ...
            for (const [index, helper] of helpers.entries()) {
                ...
                const record = JSON.parse(readFileSync(join(registryDir, records[0]), 'utf8'));
