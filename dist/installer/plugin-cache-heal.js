 ...
 */
export function buildPluginSkillContentMap(pluginRoots) {
    const map = new Map();
 ...
 */
export function pruneOrphanStandaloneSkills(options) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            options.log?.(`Removed orphan standalone skill ${entry} (content matched plugin copy; backup at .omc-trash/${entry}.${stamp})`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
...
//# sourceMappingURL=plugin-cache-heal.js.map