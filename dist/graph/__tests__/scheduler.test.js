...
import { GraphSchedulerError, applyNodeResult, beginActivationAttempt, initializeGraphProjection, isGraphSucceeded, listReadyExecutableActivations, listReadyJoinActivations, releaseAttemptForRetry, resolveJoin, sealGraphDescriptor, } from '../index.js';
import { forkJoinDescriptor, loopDescriptor } from './fixtures.js';
function begin(projection, activationId, attemptId) {
    return beginActivationAttempt(projection, {
        activation_id: activationId,
        attempt_id: attemptId,
    });
}
function succeed(descriptor, projection, activationId, attemptId, transitionId, identities = {}, route, evidenceRefs = []) {
...
describe('graph scheduler', () => {
    ...
    it('fails closed on an undeclared route and enforces a back-edge bound per lineage', () => {
        ...
        expect(() => succeed(descriptor, projection, 'a-test-1', 'try-test-1', 'bad-route', {}, 'unknown')).toThrow(/undeclared route/i);
        projection = succeed(descriptor, projection, 'a-test-1', 'try-test-1', 't-test-1', { next_activation_ids: { 'test-fail': 'a-fix-1' } }, 'fail').projection;
        projection = begin(projection, 'a-fix-1', 'try-fix-1');
        projection = succeed(descriptor, projection, 'a-fix-1', 'try-fix-1', 't-fix-1', { next_activation_ids: { 'retry-test': 'a-test-2' } }, 'retry').projection;
        projection = begin(projection, 'a-test-2', 'try-test-2');
        projection = succeed(descriptor, projection, 'a-test-2', 'try-test-2', 't-test-2', { next_activation_ids: { 'test-fail': 'a-fix-2' } }, 'fail').projection;
        projection = begin(projection, 'a-fix-2', 'try-fix-2');
        projection = succeed(descriptor, projection, 'a-fix-2', 'try-fix-2', 't-fix-2', { next_activation_ids: { 'retry-test': 'a-test-3' } }, 'retry').projection;
        projection = begin(projection, 'a-test-3', 'try-test-3');
        projection = succeed(descriptor, projection, 'a-test-3', 'try-test-3', 't-test-3', { next_activation_ids: { 'test-fail': 'a-fix-3' } }, 'fail').projection;
        projection = begin(projection, 'a-fix-3', 'try-fix-3');
        expect(() => succeed(descriptor, projection, 'a-fix-3', 'try-fix-3', 't-fix-3', { next_activation_ids: { 'retry-test': 'a-test-4' } }, 'retry')).toThrow(/traversal bound/i);
    });
    it('creates a selected fan-out cohort and consumes its branch tokens once at join', () => {
        ...
        projection = succeed(descriptor, projection, 'act-approval', 'try-approval', 'transition-approval', { next_activation_ids: { 'approval-to-analyze': 'act-analyze' } }).projection;
        expect(listReadyExecutableActivations(descriptor, projection).map((item) => item.node_id))
            .toEqual(['analyze']);
        projection = begin(projection, 'act-analyze', 'try-analyze');
        projection = succeed(descriptor, projection, 'act-analyze', 'try-analyze', 'transition-analyze', {
        ...
        projection = succeed(descriptor, projection, 'act-a', 'try-a', 'transition-a').projection;
        expect(listReadyJoinActivations(descriptor, projection)).toEqual([]);
        projection = begin(projection, 'act-b', 'try-b');
        const completedB = succeed(descriptor, projection, 'act-b', 'try-b', 'transition-b', { join_activation_id: 'act-join' });
        projection = completedB.projection;
        expect(listReadyJoinActivations(descriptor, projection).map((item) => item.activation_id))
            .toEqual(['act-join']);
        const replay = succeed(descriptor, projection, 'act-b', 'try-b', 'transition-b', { join_activation_id: 'act-join' });
        expect(replay.replayed).toBe(true);
        expect(replay.projection).toBe(projection);
        expect(() => succeed(descriptor, projection, 'act-b', 'try-b', 'transition-b', { join_activation_id: 'different-id' })).toThrow(/fingerprint/i);
        ...
        expect(() => resolveJoin(descriptor, projection, {
            activation_id: 'act-join',
            transition_id: 'transition-join',
        ...
        projection = succeed(descriptor, projection, 'act-verify', 'try-verify', 'transition-verify', {}, undefined, [{ kind: 'test', ref: 'npm-test', summary: 'All checks passed' }]).projection;
    ...
    it('requires evidence for terminal success and keeps terminal failure non-successful', () => {
        ...
        projection = succeed(descriptor, projection, 'act-test', 'try-test', 'transition-test', { next_activation_ids: { 'test-pass': 'act-verify' } }, 'pass').projection;
        projection = begin(projection, 'act-verify', 'try-verify');
        expect(() => succeed(descriptor, projection, 'act-verify', 'try-verify', 'transition-no-evidence')).toThrow(/fresh evidence/i);
