...
function useTempWorktree(sessionId) {
...
function linearDescriptor(opts = {}) {
    return {
...
function patchedDescriptor(base, revisionId, goal) {
    return {
        ...base,
...
function seedRunning(sessionId, descriptor, startNodeId = 'start', projectionOverride) {
...
async function exec(operation, input) {
    return graphCommandService.execute({ operation: operation, cwd: process.cwd(), input });
}
async function claimOne(sessionId, revisionId, descriptorHash, runId, expectedSeq, transitionId, driverId = 'driver-1') {
...
describe('red-team state-machine regressions', () => {
    it('#2: abandon atomically fences live claims and rejects a late completion against the cancelled graph', async () => {
        ...
        expect(before.claims[claim.lease_id].status).toBe('live');
        ...
        expect(afterAbandon.claims[claim.lease_id].status).toBe('fenced');
        expect(afterAbandon.claims[claim.lease_id].fenced_at).toBeDefined();
        ...
        expect(afterLate.projection.activations[claim.activation_id].status).not.toBe('completed');
    });
    ...
    it('#3: after settle-session, a worker can re-claim the previously-stranded activation', async () => {
        ...
        expect(afterSettle.claims[claim.lease_id].status).toBe('abandoned_retryable');
        ...
        expect(store.read().claims[reclaimed.lease_id].status).toBe('live');
    ...
    it('#4: dispatch_generation fence is threaded through active-scope ops across a full patch cycle', async () => {
        ...
        expect(afterPatch1.claims[startClaim.lease_id].status).toBe('completed');
    ...
    it('#5: human-approval claim returns the persisted lease expiry, not the issue timestamp', async () => {
        ...
        const persisted = store.read().claims[claim.lease_id];
        ...
        expect(Date.parse(claim.expires_at)).toBe(Date.parse(persisted.issued_at) + 60 * 60 * 1000);
    });
});
//# sourceMappingURL=red-team-regressions.test.js.map