...
function createStore(occCommit = occCommitMutation) {
...
describe('GraphStateStore', () => {
    ...
    it('commits one complete generation and replays an exact transition without re-running the callback', () => {
        const occCommit = vi.fn(occCommitMutation);
    ...
    it('fails closed for a reused transition ID with a different request fence', () => {
        ...
        expect(() => store.mutate({ ...base, operation_fingerprint: 'complete:different' }, (state) => ({ next: state, result: 'wrong' }))).toThrowError(GraphStoreError);
    });
