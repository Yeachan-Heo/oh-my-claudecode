...
import { ControlOwnerError, ControlOwnerStore, classifyControlMode, } from '../control-owner.js';
const temporaryDirectories = [];
const GRAPH_REVISION = { revision_id: 'revision-1', revision_hash: 'a'.repeat(64) };
function createStore(events = [], withExclusiveLock = (_path, callback) => {
    events.push('lock');
...
describe('ControlOwnerStore', () => {
    ...
    it('gives concurrent root reservations exactly one winner', async () => {
        ...
        const results = await Promise.allSettled(['graph', 'ralph', 'autopilot', 'team'].map((mode, index) => Promise.resolve().then(() => store.reserveRoot({
            mode,
            run_id: `${mode}-run`,
            nonce: `nonce-${index}`,
            reserved_at: '2026-07-21T00:00:00.000Z',
    ...
    it('implements the full Autopilot and Ralph child matrix and rejects mismatched parents', () => {
        ...
        for (const mode of ['ralph', 'team', 'ultrawork', 'ultraqa', 'ralplan', 'deep-interview']) {
        ...
        for (const mode of ['team', 'ultrawork', 'ultraqa', 'ralplan']) {
            ralph.registerChild({
                mode,
                session_id: `${mode}-session`,
                run_id: `${mode}-run`,
...
//# sourceMappingURL=control-owner.test.js.map