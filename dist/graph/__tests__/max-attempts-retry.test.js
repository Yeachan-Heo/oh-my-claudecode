...
import { GraphSchedulerError, applyNodeResult, beginActivationAttempt, initializeGraphProjection, isGraphSucceeded, listReadyExecutableActivations, releaseAttemptForRetry, } from '../index.js';
...
function retryDescriptor(maxAttempts) {
    return {
...
describe('A2: beginActivationAttempt enforces max_attempts', () => {
    it('throws max_attempts_exceeded when a release+reclaim would exceed the bound', () => {
        ...
        expect(thrown.code).toBe('max_attempts_exceeded');
        expect(String(thrown.message)).toMatch(/exceeds max_attempts/);
    });
...
describe('A3: applyNodeResult makes a failed activation retryable or terminal-failed', () => {
    function failAttempt(descriptor, projection, activationId, attemptId, transitionId) {
...
describe('A3: runtime promotes the graph to failed when an exhausted node fails', () => {
    ...
    function seedRunning(sessionId, descriptor) {
    ...
    it('max_attempts:1 - a single failure reaches terminal graph status failed (no wedge)', async () => {
        ...
        expect(after.projection.activations[claim.activation_id].status).toBe('failed');
    ...
    it('max_attempts:2 - first failure stays retryable, second failure promotes to failed', async () => {
        ...
        expect(afterFirst.projection.activations[claim1.activation_id].status).toBe('ready');
        ...
        expect(afterSecond.projection.activations[claim1.activation_id].status).toBe('failed');
    });
});
//# sourceMappingURL=max-attempts-retry.test.js.map