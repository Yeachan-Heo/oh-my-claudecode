...
import { applyNodeResult, beginActivationAttempt, initializeGraphProjection, sealGraphDescriptor, } from '../index.js';
...
function projectionAfterOneBackEdgeTraversal(descriptor) {
...
describe('graph state traversal_counts persistence (blocker A1)', () => {
    it('round-trips a persisted back-edge traversal count and allows a second traversal', () => {
        ...
        const [parsedOwner, parsedEdge] = JSON.parse(traversalKeys[0]);
        expect(parsedOwner).toBe(ownerActivationId);
        expect(parsedEdge).toBe('retry-test');
        expect(projection.traversal_counts[traversalKeys[0]]).toBe(1);
        ...
        expect(roundTripped.projection.traversal_counts[traversalKeys[0]]).toBe(1);
        ...
        replayed = succeed(descriptor, replayed, 'a-test-2', 'try-test-2', 't-test-2', { next_activation_ids: { 'test-fail': 'a-fix-2' } }, 'fail').projection;
        replayed = begin(replayed, 'a-fix-2', 'try-fix-2');
        expect(() => succeed(descriptor, replayed, 'a-fix-2', 'try-fix-2', 't-fix-2', { next_activation_ids: { 'retry-test': 'a-test-3' } }, 'retry')).not.toThrow();
        // The second back-edge traversal must have incremented the persisted counter.
        expect(replayed.traversal_counts[traversalKeys[0]]).toBe(1);
        const afterSecond = succeed(descriptor, replayed, 'a-fix-2', 'try-fix-2', 't-fix-2', { next_activation_ids: { 'retry-test': 'a-test-3' } }, 'retry').projection;
        expect(afterSecond.traversal_counts[traversalKeys[0]]).toBe(2);
    });
});
//# sourceMappingURL=traversal-counts-persistence.test.js.map