...
import { GraphDescriptorValidationError, canonicalJson, computeDescriptorHash, parseGraphDescriptor, sealGraphDescriptor, verifyDescriptorHash, } from '../index.js';
...
describe('graph descriptor', () => {
    it('strictly parses all four node kinds and seals the exact revision', () => {
        ...
        expect(new Set(sealed.nodes.map((node) => node.kind))).toEqual(new Set(['agent', 'command', 'human-approval', 'join']));
    });
    ...
    it('accepts branch-local returns and rejects fork region crossings', () => {
        ...
        local.edges.push({ id: 'a-to-check', kind: 'fixed', from: 'branch-a', to: 'branch-a-check' }, {
            id: 'a-check-pass',
            kind: 'conditional',
            from: 'branch-a-check',
            to: 'join-build',
        ...
        if (returnEdge?.kind === 'back_edge')
            returnEdge.to = 'branch-b';
    ...
    it('rejects nested forks and accepts sequential fork/join regions', () => {
        ...
        nested.nodes.push(executableNode('nested-a'), executableNode('nested-b'), {
            id: 'nested-join',
            kind: 'join',
            title: 'Nested join',
        ...
        nested.edges.push({
            ...
            branch_id: 'nested-b',
            owner_join_id: 'nested-join',
        }, { id: 'nested-a-join', kind: 'fixed', from: 'nested-a', to: 'nested-join' }, { id: 'nested-b-join', kind: 'fixed', from: 'nested-b', to: 'nested-join' }, { id: 'nested-out', kind: 'fixed', from: 'nested-join', to: 'join-build' });
        expect(() => parseGraphDescriptor(nested)).toThrow(/nested|fork region/i);
        const sequential = forkJoinDescriptor();
        sequential.nodes.push(executableNode('fan-second'), executableNode('second-a'), executableNode('second-b'), {
            id: 'join-second',
            kind: 'join',
            title: 'Second join',
        ...
        sequential.edges.push({ id: 'first-to-second', kind: 'fixed', from: 'join-build', to: 'fan-second' }, {
            id: 'second-fan-a',
            kind: 'fan_out',
            from: 'fan-second',
            to: 'second-a',
            ...
            branch_id: 'b2',
            owner_join_id: 'join-second',
        }, { id: 'second-a-join', kind: 'fixed', from: 'second-a', to: 'join-second' }, { id: 'second-b-join', kind: 'fixed', from: 'second-b', to: 'join-second' }, { id: 'second-to-verify', kind: 'fixed', from: 'join-second', to: 'verify' });
