...
export const GRAPH_STATE_FORMAT_VERSION = 1;
...
export function createInitialGraphState(input) {
    const descriptor = parseGraphDescriptor(input.descriptor);
    if (!verifyDescriptorHash(descriptor))
        throw new Error('Initial graph descriptor must be sealed');
    if (input.approval
    ...
    if (!input.approval && Object.values(input.projection.activations).some((activation) => activation.status === 'ready' || activation.status === 'running')) {
...
function requireRecord(value, name) {
    if (!isRecord(value))
        throw new GraphStateValidationError(`${name} must be an object`);
    return value;
}
function requireString(value, name, max = 32_768) {
...
function requireTimestamp(value, name) {
    ...
    if (!Number.isFinite(Date.parse(result)))
        throw new GraphStateValidationError(`${name} must be an ISO timestamp`);
    return result;
}
function assertExactKeys(record, required, optional, name) {
    ...
    if (missing.length > 0)
        throw new GraphStateValidationError(`${name} missing ${missing.join(', ')}`);
    if (unknown.length > 0)
        throw new GraphStateValidationError(`${name} has unknown ${unknown.join(', ')}`);
}
function requireStringArray(value, name, max) {
    ...
    if (new Set(result).size !== result.length)
        throw new GraphStateValidationError(`${name} contains duplicates`);
    return result;
}
function validateEvidence(value, name) {
    ...
    if (evidence.summary !== undefined)
        requireString(evidence.summary, `${name}.summary`, 2_048);
    return value;
}
...
function validateApproval(value, name, revisionId, revisionHash) {
    ...
    if (evidence.kind !== 'human')
        throw new GraphStateValidationError(`${name} requires human evidence`);
    return value;
}
function validateProjection(value, descriptor) {
    ...
    for (const [key, raw] of Object.entries(activations)) {
        ...
        if (activation.activation_id !== key)
            throw new GraphStateValidationError(`activation key ${key} mismatches identity`);
        const nodeId = requireString(activation.node_id, `projection.activations.${key}.node_id`, 128);
        if (!nodeIds.has(nodeId))
            throw new GraphStateValidationError(`activation ${key} references unknown node ${nodeId}`);
        ...
        if (attemptNo !== attempts.length)
            throw new GraphStateValidationError(`activation ${key} attempt count mismatch`);
        ...
        if (activation.active_attempt_id !== undefined) {
            ...
            if (!attempts.includes(activeAttempt))
                throw new GraphStateValidationError(`activation ${key} active attempt is unknown`);
        }
    ...
    for (const [key, raw] of Object.entries(cohorts)) {
        ...
        if (cohort.cohort_id !== key)
            throw new GraphStateValidationError(`cohort key ${key} mismatches identity`);
        for (const property of ['fan_out_node_id', 'owner_join_id']) {
            ...
            if (!nodeIds.has(nodeId))
                throw new GraphStateValidationError(`cohort ${key} references unknown node ${nodeId}`);
        }
        requireStringArray(cohort.expected_branch_token_ids, `projection.cohorts.${key}.expected_branch_token_ids`, 64);
        if (typeof cohort.consumed !== 'boolean')
            throw new GraphStateValidationError(`cohort ${key}.consumed must be boolean`);
    ...
    for (const [key, raw] of Object.entries(tokens)) {
        ...
        if (token.branch_token_id !== key)
            throw new GraphStateValidationError(`branch token key ${key} mismatches identity`);
        if (!(String(token.cohort_id) in cohorts))
            throw new GraphStateValidationError(`branch token ${key} references unknown cohort`);
    ...
    const traversalOwnerIds = new Set([...Object.keys(activations), ...Object.keys(tokens)]);
    ...
    for (const [key, raw] of Object.entries(committed)) {
        ...
        if (transition.transition_id !== key)
            throw new GraphStateValidationError(`scheduler transition key ${key} mismatches identity`);
        ...
        if (!nodeIds.has(nodeId))
            throw new GraphStateValidationError(`scheduler transition ${key} references unknown node`);
    ...
    const terminalIds = requireStringArray(projection.terminal_verification_activation_ids, 'projection.terminal_verification_activation_ids', GRAPH_MAX_TRANSITIONS);
...
function validateRevision(value, key, runId) {
    ...
    if (!/^[a-f0-9]{64}$/.test(hash))
        throw new GraphStateValidationError(`revision ${key} hash is invalid`);
    if (revisionId !== key)
        throw new GraphStateValidationError(`revision key ${key} does not match revision_id`);
    ...
    if (revision.approval !== undefined)
        validateApproval(revision.approval, `revisions.${key}.approval`, key, hash);
    return value;
}
export function parseGraphState(input) {
    let serialized;
    try {
    ...
    if (!/^[a-f0-9]{64}$/.test(activeHash))
        throw new GraphStateValidationError('active_revision_hash is invalid');
    ...
    for (const raw of state.transitions) {
        ...
        if (transitionIds.has(id))
            throw new GraphStateValidationError(`duplicate transition ${id}`);
        transitionIds.add(id);
        const currentSequence = requireInteger(transition.sequence, 'transition.sequence', GRAPH_MAX_TRANSITIONS);
        if (currentSequence <= priorSequence)
            throw new GraphStateValidationError('transition sequence is not monotonic');
    ...
    if (Object.keys(claims).length > GRAPH_MAX_CLAIMS)
        throw new GraphStateValidationError('claims exceed the configured bound');
    ...
    for (const [leaseId, raw] of Object.entries(claims)) {
        ...
        if (claim.lease_id !== leaseId)
            throw new GraphStateValidationError(`claim key ${leaseId} mismatches identity`);
        if (claim.run_id !== runId)
            throw new GraphStateValidationError(`claim ${leaseId} run fence mismatches`);
        ...
        if (!activation)
            throw new GraphStateValidationError(`claim ${leaseId} references unknown activation`);
        ...
        if (renewals > maxRenewals)
            throw new GraphStateValidationError(`claim ${leaseId} renewal cap is exceeded`);
        ...
        if (claim.fenced_at !== undefined)
            requireTimestamp(claim.fenced_at, `claims.${leaseId}.fenced_at`);
    ...
    for (const [id, raw] of Object.entries(reconciliations)) {
        ...
        if (record.reconciliation_id !== id)
            throw new GraphStateValidationError(`reconciliation key ${id} mismatches identity`);
    ...
    state.diagnostics.forEach((raw, index) => {
        ...
        for (const property of ['activation_id', 'attempt_id', 'lease_id']) {
            if (diagnostic[property] !== undefined)
                requireString(diagnostic[property], `diagnostics[${index}].${property}`, 128);
        }
    });
    ...
    if (!active.approval && Object.values(state.projection.activations).some((activation) => activation.status === 'ready' || activation.status === 'running')) {
    ...
    if (Object.values(claims).some((raw) => raw.claim_owner_session_id !== sessionId)) {
