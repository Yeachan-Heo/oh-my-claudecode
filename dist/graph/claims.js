...
import { GRAPH_MAX_DIAGNOSTICS, GRAPH_MAX_RECONCILIATIONS, parseGraphState, } from './runtime-types.js';
...
function timestamp(value, name) {
    ...
    if (!Number.isFinite(parsed))
        throw new GraphClaimError('invalid_timestamp', `${name} must be an ISO timestamp`);
    return parsed;
}
...
function findClaim(state, leaseId) {
    const claim = state.claims[leaseId];
    if (!claim)
        throw new GraphClaimError('claim_not_found', `Claim ${leaseId} does not exist`);
    return claim;
}
function ensureLiveClaim(state, leaseId) {
    const claim = findClaim(state, leaseId);
    if (claim.status !== 'live')
        throw new GraphClaimError('claim_not_live', `Claim ${leaseId} is no longer live`);
    return claim;
}
function cloneWithClaims(state, claims) {
    return { ...structuredClone(state), claims };
}
function validateLeasePolicy(input) {
...
export function issueGraphClaim(stateInput, input) {
    ...
    if (state.claims[input.lease_id])
        throw new GraphClaimError('duplicate_lease', `Lease ${input.lease_id} already exists`);
    if (Object.values(state.claims).some((claim) => claim.activation_id === input.activation_id && claim.status === 'live')) {
...
export function renewGraphClaim(stateInput, input) {
    const state = parseGraphState(stateInput);
...
function reconciliationFor(claim, input, reason) {
    return {
...
export function recoverExpiredGraphClaim(stateInput, input, replaceAttempt) {
    const state = parseGraphState(stateInput);
    const claim = ensureLiveClaim(state, input.lease_id);
    const now = timestamp(input.now, 'now');
    if (now < timestamp(claim.expires_at, 'claim.expires_at')) {
    ...
    if (claim.effect_policy.policy === 'reconcile') {
        ...
        const fenced = { ...claim, status: 'reconciling', fenced_at: input.now };
        const next = {
    ...
    if (state.claims[input.new_lease_id])
        throw new GraphClaimError('duplicate_lease', 'Replacement lease already exists');
...
export function recordLateClaimResult(stateInput, input) {
    const state = parseGraphState(stateInput);
...
export function settleDriverClaims(stateInput, input) {
    const state = parseGraphState(stateInput);
    timestamp(input.recorded_at, 'recorded_at');
    ...
    for (const claim of Object.values(claims)) {
        if (claim.status !== 'live'
            || claim.claim_owner_session_id !== input.claim_owner_session_id)
            continue;
        // In 'driver' scope only fence claims owned by the passed driver. In
        // 'session' scope fence every live claim in the session.
        if (scope === 'driver' && claim.driver_instance_id !== input.driver_instance_id)
            continue;
    ...
    const next = {
        ...structuredClone(state),
        ...(hasAmbiguous ? { status: 'reconciling' } : {}),
        ...(resumeRunning ? { status: 'running' } : {}),
        claims,
        reconciliations,
