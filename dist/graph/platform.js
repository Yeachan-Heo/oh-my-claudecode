...
function parseLinuxProcessStart(stat) {
...
function readProcessStartLinux(pid, deps) {
    try {
...
function readProcessStartDarwin(pid, deps) {
    try {
...
function readProcessStartWindows(pid, deps) {
    try {
        const output = deps.execCommand(`powershell -NoProfile -Command "[math]::Floor(((Get-Process -Id ${pid} -ErrorAction SilentlyContinue).StartTime - (Get-Date '1970-01-01')).TotalSeconds)"`);
        const trimmed = output.trim();
...
function readProcessStart(pid, platform, deps) {
    if (platform === 'linux')
        return readProcessStartLinux(pid, deps);
    if (platform === 'darwin')
        return readProcessStartDarwin(pid, deps);
    if (platform === 'win32')
        return readProcessStartWindows(pid, deps);
    return null;
}
...
// liveness semantics.
const warnedPidOnlyFallback = new Set();
export function createGraphPlatformAdapter(overrides = {}) {
    const dependencies = {
        platform: process.platform,
        pid: process.pid,
        fileExists: existsSync,
        readText: (path) => readFileSync(path, 'utf8'),
        execCommand: (command) => execSync(command, { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], timeout: 2000 }),
        ...overrides,
    };
    return {
        preflight() {
        ...
        isProcessIdentityLive(identity) {
            ...
            if (!identity.process_start) {
                if (!warnedPidOnlyFallback.has(identity.pid)) {
                    ...
                    console.warn(`graph: pid-only liveness fallback used for pid ${identity.pid} (process_start unavailable); PID-reuse protection is degraded`);
                }
                return true;
            }
