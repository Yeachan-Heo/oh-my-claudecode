...
export async function startTeamV2(config) {
    ...
    for (const { agentType, reason } of missingBinaryReasons) {
        process.stderr.write(`[team/runtime-v2] cli_binary_missing:${agentType}: ${reason} — provider route unavailable\n`);
...
export async function shutdownTeamV2(teamName, cwd, options = {}) {
    ...
    for (const worker of config.workers) {
        if (!worker.launch_attempt_id || !worker.pane_id)
            continue;
    ...
    try {
        ...
        if (splitPaneMode) {
            await killTeamSession(config.tmux_session, workerPaneIds, config.leader_pane_id ?? undefined, { sessionMode: 'split-pane' });
        }
        else {
