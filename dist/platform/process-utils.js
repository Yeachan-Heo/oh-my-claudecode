...
export function linuxProcStateFromStat(stat) {
...
function linuxProcRecordFromStat(stat) {
    ...
    if (!state || state.length !== 1 || !Number.isInteger(processGroupId) || processGroupId <= 0)
        return null;
 ...
 */
export function isProcessGroupQuiescent(processGroupId) {
    ...
    if (process.platform === 'linux') {
        try {
            for (const entry of readdirSync('/proc')) {
                ...
                if (!record)
                    return false;
                if (record.processGroupId === processGroupId && record.state !== 'Z' && record.state !== 'X')
                    return false;
            }
            return true;
        }
