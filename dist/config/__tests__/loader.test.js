...
describe("team.roleRouting (Option E)", () => {
    ...
    it("accepts gjc as team defaultAgentType and roleRouting provider", () => {
        const tempDir = mkdtempSync(join(tmpdir(), "omc-team-routing-gjc-"));
        try {
            const claudeDir = join(tempDir, ".claude");
            require("node:fs").mkdirSync(claudeDir, { recursive: true });
            writeFileSync(join(claudeDir, "omc.jsonc"), JSON.stringify({
                team: {
                    ops: { defaultAgentType: "gjc" },
                    roleRouting: {
                        executor: { provider: "gjc" },
                    },
                },
            }));
            process.chdir(tempDir);
            const config = loadConfig();
            expect(config.team?.ops?.defaultAgentType).toBe("gjc");
            expect(config.team?.roleRouting?.executor).toEqual({ provider: "gjc" });
        }
        finally {
            rmSync(tempDir, { recursive: true, force: true });
        }
