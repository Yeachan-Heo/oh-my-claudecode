...
function writeKeychainCredentials(creds) {
    ...
    try {
        const serviceName = getKeychainServiceName();
        const existing = execSync(`/usr/bin/security find-generic-password -s "${serviceName}" -w 2>/dev/null`, { encoding: 'utf-8', timeout: 2000 }).trim();
        if (!existing)
            return false;
        ...
        execSync(`/usr/bin/security add-generic-password -s "${serviceName}" -a "Claude Code" -w ${JSON.stringify(updated)} -U`, { timeout: 2000 });
        return true;
    }
    catch {
        return false;
