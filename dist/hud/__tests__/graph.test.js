...
import { getActiveSkills, isAnyModeActive, readGraphStateForHud, } from '../omc-state.js';
function graphState(sessionId) {
...
describe('Graph HUD projection', () => {
    ...
    it.each(['paused', 'failed', 'cancelled', 'succeeded'])('hides terminal or inactive status %s', (status) => {
        const directory = mkdtempSync(join(tmpdir(), 'omc-graph-hud-'));
        directories.push(directory);
