...
import { OPENCLAW_DEFAULT_MODEL, spawnOpenclawAgent, sendToSession, getOpenclawInfo, isGatewayRunning } from './openclaw-core.js';
const spawnAgentTool = tool("spawn_agent", "Spawn an OpenClaw sub-agent to perform a task in an isolated, parallel session. Returns immediately with sessionKey.", {
...
const sendMessageTool = tool("send_to_session", "Send a message to an existing, active OpenClaw session.", {
    session_key: { type: "string", description: "The key of the target session (from spawn_agent)." },
    message: { type: "string", description: "The message to send." },
}, async (args) => {
    const { session_key, message } = args;
    const result = await sendToSession(session_key, message);
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
const getInfoTool = tool("get_info", "Get diagnostic information about the OpenClaw installation.", {}, async () => {
    const result = getOpenclawInfo();
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
const isReadyTool = tool("is_ready", "Check if the OpenClaw gateway is running and ready to accept commands.", {}, async () => {
...
//# sourceMappingURL=openclaw-server.js.map