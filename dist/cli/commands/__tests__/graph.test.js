...
import { GRAPH_COMMAND_OPERATIONS, graphCommand, } from '../graph.js';
...
function serviceReturning(result = { state: 'ok' }) {
    const requests = [];
    return {
        requests,
        service: {
...
const TRANSITION = ['--transition-id', 'transition-1'];
const COMMON = [
    '--run-id', 'run-1',
...
const SEQUENCED = [...COMMON, '--expected-sequence', '7'];
const DRIVER = ['--driver-id', 'driver-1'];
...
describe('omc graph CLI boundary', () => {
    ...
    it('forwards complete identities through the CLI boundary so a fan-out join activates', async () => {
        ...
        try {
            ...
            const execute = (operation, input) => graphCommandService.execute({
                operation: operation,
                cwd: process.cwd(),
                input,
            });
            ...
            const claim = async (expectedSequence, transitionId) => {
            ...
            const complete = async (expectedSequence, transitionId, currentClaim, evidenceKind) => execute('complete', {
                ...scope,
                expected_sequence: expectedSequence,
                transition_id: transitionId,
                claim: {
    ...
    it('rejects omitted session scope and mutating transition identity', async () => {
        ...
        captured.err.length = 0;
        process.exitCode = 0;
        await graphCommand(['claim', ...SEQUENCED, ...DRIVER, '--limit', '3'], service);
        expect(process.exitCode).toBe(1);
    ...
    it('ships an explicit-only persistent in-session driver contract', async () => {
        const skill = await readFile(new URL('../../../../skills/graph/SKILL.md', import.meta.url), 'utf8');
