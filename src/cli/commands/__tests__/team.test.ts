...
describe('teamCommand api operations', () => {
  ...
  it('supports claim-safe lifecycle: create -> claim -> transition', async () => {
    ...
    try {
      ...
      expect(claimed.data.claim_token).toBe(claimToken);
...
describe('parseTeamArgs comma-separated multi-type specs', () => {
  ...
  it('parses copilot as an explicit executor-path worker type', () => {
    const parsed = parseTeamArgs(['1:copilot:executor', 'implement feature']);
    expect(parsed.workerCount).toBe(1);
    expect(parsed.agentTypes).toEqual(['copilot']);
    expect(parsed.workerSpecs).toEqual([{ agentType: 'copilot', role: 'executor' }]);
    expect(parsed.role).toBe('executor');
  });
  ...
  it('keeps copilot out of configured defaultAgentType fallback resolution', () => {
    const parsed = parseTeamArgs(['run all tests'], 'copilot');
    expect(parsed.agentTypes).toEqual(['claude', 'claude', 'claude']);
    expect(parsed.workerSpecs).toEqual([
      { agentType: 'claude' },
      { agentType: 'claude' },
  ...
  it('keeps autogenerated team names valid for long multiline prompts that would otherwise end on a hyphen', () => {
    const multilineTask = [
      '1. Write COPILOTMIX to .omc/tmp/copilot-mixed-c.txt and report done',
      '2. Write CODEXMIX to .omc/tmp/codex-mixed-d.txt and report done',
    ].join('\n');

    const parsed = parseTeamArgs(['1:copilot:executor,1:codex:executor', multilineTask]);
    expect(parsed.teamName).toMatch(/^[a-z0-9][a-z0-9-]{0,48}[a-z0-9]$/);
