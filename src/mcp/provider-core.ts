...
export const PLATFORM_SHELL_OPTS: Record<string, boolean> =
  process.platform === 'win32' ? { shell: true } : {};
export const RATE_LIMIT_PATTERN =
  /429|rate.?limit|too many requests|quota.?exceeded|resource.?exhausted/i;
...
export function createPidRegistry(): PidRegistry {
  const pids = new Set<number>();
  return {
    add: (pid: number) => { pids.add(pid); },
    delete: (pid: number) => { pids.delete(pid); },
    has: (pid: number) => pids.has(pid),
    clear: () => { pids.clear(); },
  };
}
...
export function executeCli(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (data: Buffer) => { collector.append(data.toString()); });
  ...
  if (modelExplicit && rlConfig) {
    ...
    for (let attempt = 0; attempt <= rlConfig.retryCount; attempt++) {
      try {
        ...
        if (!RATE_LIMIT_PATTERN.test(lastError.message)) throw lastError;
...
export function executeCliBackground(
  ...
  try {
    ...
    const trySpawnWithModel = (
      ...
      const handleRetryable = (retryableErr: RetryableErrorResult): void => {
 ...
 */
export async function handleAskProvider(
  config: ProviderConfig,
  pidRegistry: PidRegistry,
  args: ProviderAskArgs,
): Promise<McpResponse> {
  ...
  if (pathPolicy === 'strict') {
    ...
    if (worktreeRoot) {
      ...
      try { worktreeReal = realpathSync(worktreeRoot); } catch { worktreeReal = ''; }
      if (worktreeReal) {
  ...
  if (!isValidAgentRoleName(agent_role)) {
    return singleErrorBlock(
      `Invalid agent_role: "${agent_role}". Role names must contain only lowercase letters, numbers, and hyphens. ` +
      `Recommended for ${config.name}: ${config.recommendedRoles.join(', ')}`,
    );
  }
  if (!VALID_AGENT_ROLES.includes(agent_role)) {
    return singleErrorBlock(
      `Unknown agent_role: "${agent_role}". Available roles: ${VALID_AGENT_ROLES.join(', ')}. ` +
      `Recommended for ${config.name}: ${config.recommendedRoles.join(', ')}`,
    );
  }
  ...
  if (args.background) {
    ...
    const result = executeCliBackground(config, pidRegistry, fullPrompt, args.model as string | undefined, {
      provider: config.name,
    ...
    return { content: [{ type: 'text' as const, text: bgLines.join('\n') }] };
  }
  ...
  try {
    const { response, usedFallback, actualModel } = await executeCliWithFallback(
      config, fullPrompt, args.model as string | undefined, baseDir, fallbackChain, extra,
    );
    ...
    return singleErrorBlock(`${paramLines}\n\n---\n\n${config.name} CLI error: ${(err as Error).message}`);
  }
}
