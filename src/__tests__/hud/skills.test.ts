...
describe('renderSkills', () => {
  ...
  describe('namespaced skill names', () => {
    it('displays only last segment for namespaced skills (lazycc:plan)', () => {
      const lastSkill: SkillInvocation = { name: 'lazycc:plan', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain('skill:plan');
      expect(result).not.toContain('lazycc');
    });

    it('displays only last segment for namespaced skills with args', () => {
      const lastSkill: SkillInvocation = { name: 'lazycc:autopilot', args: 'build app', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
