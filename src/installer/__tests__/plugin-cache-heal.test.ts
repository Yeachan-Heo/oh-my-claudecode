...
import {
  healHardcodedHookNodePaths,
  healHooksJsonFile,
  pruneOrphanStandaloneSkills,
} from '../plugin-cache-heal.js';
...
describe('plugin-cache-heal', () => {
  ...
  const bogusNode = '/opt/hostedtoolcache/node/20.20.2/x64/bin/node';
  ...
  describe('healHooksJsonFile', () => {
    it('rewrites unreachable absolute node paths to the local node binary', () => {
            ...
            PreToolUse: [
              {
                ...
                hooks: [
                  {
                    type: 'command',
                    command: `"${bogusNode}" "$CLAUDE_PLUGIN_ROOT"/scripts/run.cjs "$CLAUDE_PLUGIN_ROOT"/scripts/pre-tool-use.mjs`,
                  },
                ],
              },
            ],
      ...
      const changed = healHooksJsonFile(hooksPath, realNode);
      expect(changed).toBe(true);

      const data = JSON.parse(readFileSync(hooksPath, 'utf-8'));
      const command = data.hooks.PreToolUse[0].hooks[0].command as string;
      expect(command.startsWith(`"${realNode}" `)).toBe(true);
      expect(command).not.toContain(bogusNode);
      expect(command).toContain('/scripts/run.cjs');
      expect(command).toContain('/scripts/pre-tool-use.mjs');
    });

    it('leaves unrelated commands untouched', () => {
      const hooksPath = join(pluginRoot, 'hooks', 'hooks.json');
      const untouched = '/usr/bin/touch /tmp/some-other-hook';
      writeFileSync(
        hooksPath,
        JSON.stringify({
          hooks: {
            PreToolUse: [
              {
                matcher: 'Bash',
                hooks: [{ type: 'command', command: untouched }],
              },
            ],
          },
        }),
      ...
      const data = JSON.parse(readFileSync(hooksPath, 'utf-8'));
      expect(data.hooks.PreToolUse[0].hooks[0].command).toBe(untouched);
    });

    it('does not rewrite an absolute path that exists on disk', () => {
      const hooksPath = join(pluginRoot, 'hooks', 'hooks.json');
      const existingNode = realNode; // process.execPath always exists
      writeFileSync(
        hooksPath,
        JSON.stringify({
          hooks: {
            PreToolUse: [
              {
                ...
                hooks: [
                  {
                    type: 'command',
                    command: `"${existingNode}" "$CLAUDE_PLUGIN_ROOT"/scripts/run.cjs "$CLAUDE_PLUGIN_ROOT"/scripts/pre-tool-use.mjs`,
                  },
                ],
              },
            ],
    ...
    it('returns false on missing file', () => {
      expect(healHooksJsonFile(join(workDir, 'nonexistent.json'), realNode)).toBe(false);
    });

    it('returns false on malformed JSON', () => {
      const hooksPath = join(pluginRoot, 'hooks', 'hooks.json');
      writeFileSync(hooksPath, '{ this is not valid json');
      expect(healHooksJsonFile(hooksPath, realNode)).toBe(false);
    });
  });

  describe('healHardcodedHookNodePaths', () => {
    it('walks all provided plugin roots and reports rewrites', () => {
      const hooksPath = join(pluginRoot, 'hooks', 'hooks.json');
      writeFileSync(
        hooksPath,
        JSON.stringify({
            ...
            UserPromptSubmit: [
              {
                hooks: [
                  {
                    command: `"${bogusNode}" "$CLAUDE_PLUGIN_ROOT"/scripts/run.cjs "$CLAUDE_PLUGIN_ROOT"/scripts/keyword-detector.mjs`,
                  },
                ],
              },
            ],
      ...
      const logs: string[] = [];
      const result = healHardcodedHookNodePaths([pluginRoot], {
        nodeBin: realNode,
        log: msg => logs.push(msg),
      });

      expect(result.scanned).toEqual([hooksPath]);
      expect(result.rewritten).toEqual([hooksPath]);
      expect(result.errors).toEqual([]);
      expect(logs.some(line => line.includes('Healed hardcoded node path'))).toBe(true);
    });

    it('skips plugin roots without a hooks.json', () => {
      const result = healHardcodedHookNodePaths([pluginRoot], { nodeBin: realNode });
