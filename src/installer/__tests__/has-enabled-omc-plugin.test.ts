...
describe('hasEnabledOmcPlugin', () => {
  ...
  describe('Modern Claude Code 1.x format (`enabledPlugins`)', () => {
    it('returns true when enabledPlugins.lazycc@lazycc is true', async () => {
      writeSettings({
        enabledPlugins: {
          'lazycc@lazycc': true,
    ...
    it('returns true for any pluginId substring matching lazycc', async () => {
      writeSettings({
        enabledPlugins: {
          'lazycc-fork@somerepo': true,
        },
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
      expect(hasEnabledOmcPlugin()).toBe(true);
    ...
    it('returns false when enabledPlugins.lazycc@lazycc is explicitly false', async () => {
      writeSettings({
        enabledPlugins: {
          'lazycc@lazycc': false,
        },
    ...
    it('returns false when enabledPlugins has no lazycc entry', async () => {
      writeSettings({
        enabledPlugins: {
          'unrelated-plugin@foo': true,
        },
    ...
    it('handles enabledPlugins as an array of plugin id strings', async () => {
      writeSettings({
        enabledPlugins: ['lazycc@lazycc', 'other'],
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
      expect(hasEnabledOmcPlugin()).toBe(true);
    });
  ...
  describe('Legacy `plugins` field (backward compatibility)', () => {
    it('returns true when plugins.lazycc@lazycc is true', async () => {
      writeSettings({
        plugins: {
          'lazycc@lazycc': true,
        },
    ...
    it('returns true when plugins is an array with lazycc entry', async () => {
      writeSettings({
        plugins: ['lazycc'],
      });
      const { hasEnabledOmcPlugin } = await freshInstaller();
  ...
  describe('Mixed format support', () => {
    ...
    it('returns true when modern enabledPlugins has the entry but legacy plugins does not', async () => {
      writeSettings({
        enabledPlugins: { 'lazycc@lazycc': true },
