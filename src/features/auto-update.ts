...
function tryExec(command: string, options: { timeout?: number; stdio?: any } = {}): { ok: boolean; output?: string; error?: string } {
  try {
...
function syncMarketplaceClone(verbose: boolean = false): { ok: boolean; message: string } {
  ...
  if (!fetch.ok) return { ok: false, message: `Failed to fetch marketplace clone: ${fetch.error}` };
  ...
  if (!pull.ok) return { ok: false, message: `Failed to update marketplace clone: ${pull.error}` };
