...
function parseMergeReadinessSourceMode(promptText: string): "diff" | "artifacts" | undefined {
  if (promptText.includes("--from-diff")) return "diff";
  if (promptText.includes("--from-artifacts")) return "artifacts";
  return undefined;
}

function hasMinimalEvidenceForMode(evidence: MergeReadinessEvidence, mode: "diff" | "artifacts" | undefined): boolean {
  const hasDiff = evidence.changedFiles.length > 0 || Boolean(evidence.status) || Boolean(evidence.diffStat);
...
export function createInitialMergeReadinessState(
  ...
  const sourceMode = parseMergeReadinessSourceMode(promptText);
  const evidence = collectMergeReadinessEvidence(directory, baseRef);
  const missingEvidence = unsupportedFromPr || !hasMinimalEvidenceForMode(evidence, sourceMode);
...
export function setMergeReadinessContent(
  ...
  const now = new Date().toISOString();
  // Do not re-arm a blocked no-evidence state into a quiz.
  if (state.result === "blocked" && !hasMinimalEvidenceForMode(state.evidence, state.source_mode)) {
    state.validation_errors = ["Blocked: no minimal change evidence (diff/status/artifacts). Produce the change before submitting content."];
...
export function overrideMergeReadiness(directory: string, reason: string, sessionId?: string): MergeReadinessState | null {
  ...
  if (state.result === "blocked" && !hasMinimalEvidenceForMode(state.evidence, state.source_mode)) {
    state.validation_errors = ["Blocked: no minimal change evidence; override is not permitted. Produce the change before overriding."];
    return state;
  }
  state.active = false;
  state.phase = "complete";
