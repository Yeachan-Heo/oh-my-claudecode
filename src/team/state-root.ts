...
export function resolveCanonicalTeamStateRoot(
  ...
  if (explicit) return resolve(leaderCwd, explicit);
...
type TeamRootLayout = 'state_root' | 'team_root';

function readExplicitTeamStateRoot(env: NodeJS.ProcessEnv): string {
...
function readLeaderCwd(env: NodeJS.ProcessEnv): string {
...
function teamRootPath(stateRoot: string, teamName: string, layout: TeamRootLayout): string {
  return layout === 'team_root' ? stateRoot : join(stateRoot, 'team', teamName);
}

async function candidateLayouts(resolvedStateRoot: string, teamName: string): Promise<TeamRootLayout[]> {
  const layouts: TeamRootLayout[] = [];
  if (await pathIsDirectory(join(resolvedStateRoot, 'team', teamName))) layouts.push('state_root');
...
async function readMetadataRootFromValidatedCandidate(
  ...
  filename: 'identity.json' | 'manifest.json' | 'manifest.v2.json' | 'config.json',
  cwd: string,
  worker: TeamWorkerIdentityRef,
): Promise<string | null> {
...
async function readTeamMetadataRootFromCandidate(
  candidateStateRoot: string,
  filename: 'manifest.json' | 'manifest.v2.json' | 'config.json',
  cwd: string,
  worker: TeamWorkerIdentityRef,
): Promise<string | null> {
  const resolvedStateRoot = resolve(cwd, candidateStateRoot);
...
async function resolveWorkerTeamStateRootWithOptions(
  ...
  const metadataSources: Array<[
    'identity.json' | 'manifest.json' | 'manifest.v2.json' | 'config.json',
