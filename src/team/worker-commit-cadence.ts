...
export interface WorkerCadenceContext {
  ...
  workerName: string;
  worktreePath: string;
  agentType: 'claude' | 'codex' | 'gemini' | 'cursor' | 'grok' | 'gjc';
  enabled: boolean;
}
 ...
 * - codex / gemini / cursor / gjc → fallback fs-watch poller (caller owns the handle)
 *
...
export async function installCommitCadence(
  ...
  // codex / gemini / cursor / gjc: no PostToolUse hook; caller starts the fallback poller.
