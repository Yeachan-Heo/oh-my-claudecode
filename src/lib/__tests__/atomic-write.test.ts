...
describe('lease-based mutation lock: validateMutationLockOwner', () => {
  ...
  it('rejects an owner with extra keys, wrong types, bad nonce, or missing expires_at', () => {
    ...
    // Missing expires_at (the old liveness schema) is NOT a valid lease owner.
    expect(validateMutationLockOwner({ version: 1, pid: 1, createdAt: new Date().toISOString(), nonce: randomUUID() })).toBeNull();
    expect(validateMutationLockOwner({ version: 1, pid: 1, createdAt: new Date().toISOString(), expires_at: 'not-a-date', nonce: randomUUID() })).toBeNull();
    expect(validateMutationLockOwner({ version: 1, pid: 1, createdAt: new Date().toISOString(), expires_at: new Date(Date.now() + 1000).toISOString(), nonce: 'not-a-uuid' })).toBeNull();
    expect(validateMutationLockOwner({ version: 2, pid: 1, createdAt: new Date().toISOString(), expires_at: new Date(Date.now() + 1000).toISOString(), nonce: randomUUID() })).toBeNull();
    expect(validateMutationLockOwner({ version: 1, pid: 1, createdAt: new Date().toISOString(), expires_at: new Date(Date.now() + 1000).toISOString(), nonce: randomUUID(), extra: 1 })).toBeNull();
  });
});
