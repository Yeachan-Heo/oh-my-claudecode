 ...
 * - Arrays with identifiable items (objects with `timestamp`+`content`/`directive`):
...
/**
 * Identity key for CustomNote: category + content (content is the unique payload).
 */
function customNoteKey(note: CustomNote): string {
  return `${note.category}::${note.content}`;
}

/**
 * Identity key for UserDirective: directive text (the unique payload).
 */
function userDirectiveKey(d: UserDirective): string {
  return d.directive;
}

/**
 * Identity key for HotPath: path (unique per tracked location).
 */
function hotPathKey(hp: HotPath): string {
  return hp.path;
}
 ...
 * - `languages`, `frameworks`, `workspaces`, `mainDirectories`: deduplicate by identity
...
export function mergeProjectMemory(
  ...
  const merged = deepMerge(existing as unknown as Record<string, unknown>, incoming as unknown as Record<string, unknown>) as unknown as ProjectMemory;
