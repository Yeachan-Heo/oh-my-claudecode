 ...
 *
 * - v1 = the OLD liveness contract: `{ version: 1, pid, processStart,
 *   createdAt, nonce }` (no `expires_at`). The owner's liveness was probed via
 *   process-start identity. A v1 record on disk is a lock held under the old
 *   contract by a process that may still be alive. We NEVER reclaim it: we
 *   fail closed (wait / return null) and surface `old_version` so an operator
 *   can decide. See B12.
 * - v2 = the LEASE contract: `{ version: 2, pid, createdAt, expires_at, nonce }`.
...
export type MutationLockOwner = { version: 2; pid: number; createdAt: string; expires_at: string; nonce: string };
...
function guardedLockRemoval(path: string, operation: 'reclaim' | 'release', owner?: MutationLockOwner): 'retry' | 'live' | 'replaced' | 'unverifiable' | 'old_version' {
    ...
    try {
      ...
      if (hasFlock) {
        ...
        if (disposition === 'old_version') {
          ...
          console.error(`[omc-lock] state_mutation_lock_old_version: ${path}`);
          return null;
        }
...
/**
 * Validates a parsed object as a lease-based (v2) MutationLockOwner using the
 * SAME checks the flock path's LOCK_REMOVAL_SCRIPT enforces: exact key set,
 * version===2, positive integer pid, parseable createdAt, parseable
 ...
 * unexpired lease.
 */
function readLockOwner(path: string): MutationLockOwner | 'absent' | 'corrupt' | 'old_version' | null {
  let raw: string;
  try {
...
export function assertMutationLockHeld(lock: MutationLock | null): void {
  ...
  if (current === 'absent' || current === 'old_version' || current === 'corrupt' || current === null) {
    // Our lock is gone / unreadable / replaced by an old-contract owner: a later
    // writer may have reclaimed after our lease expired. Treat as an overlap.
...
export function renewMutationLock(lock: MutationLock | null): boolean {
  ...
  try {
    ...
    fsyncSync(fd);
    // Atomic replace of our own lock file. linkSync over an existing path is
    // atomic on POSIX; the on-disk content swaps in one step.
    try { unlinkSync(lock.path); } catch { /* a reclaimer may have unlinked it */ }
    linkSync(tempPath, lock.path);
    unlinkSync(tempPath);
    lock.owner = renewed;
    return true;
  } catch {
