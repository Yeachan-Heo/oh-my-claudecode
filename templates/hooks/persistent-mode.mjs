      ...
      if (sessionMatches) {
        ...
        console.log(
          JSON.stringify({
            continue: false,
            decision: "block",
            reason: `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /lazycc:cancel (or --force).`,
          }),
        );
        return;
      }
