...
function resolveProjectWorkflowConfig(directory) { let current; try { current = realpathSync(resolve(directory)); } catch { current = resolve(directory); } while (true) { const config = join(current, '.claude', 'omc.jsonc'); if (existsSync(config)) return config; const parent = dirname(current); if (parent === current) return join(resolve(directory), '.claude', 'omc.jsonc'); current = parent; } }
...
export function isValidWorkflowTrackingState(state, sessionId = state?.session_id) {
  try {
    ...
    if (!hasExactKeys(tracking, ['stages', 'currentStageIndex', 'trackingRevision', 'activationBoundary', 'completionObservations']) || !Array.isArray(tracking.stages) || !Array.isArray(tracking.completionObservations) || !isFiniteInteger(tracking.currentStageIndex) || tracking.currentStageIndex > maxIndex || !isFiniteInteger(tracking.trackingRevision) || tracking.trackingRevision !== tracking.currentStageIndex) return false;
