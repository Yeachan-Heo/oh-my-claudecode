...
function inspectStateFile(path, now, thresholdMs) {
  ...
  return { ageHours: Math.round(ageHours * 10) / 10 };
}
 ...
 */
export async function findStaleRuns({ stateRoot, now = Date.now(), thresholdHours, excludeSessionId }) {
  const thresholdMs = (thresholdHours ?? staleThresholdHours()) * 3600_000;
  const stateDir = join(stateRoot, 'state');
  const entries = [];

  for (const mode of WATCHED_MODES) {
    const legacy = inspectStateFile(join(stateDir, `${mode}-state.json`), now, thresholdMs);
    if (legacy) entries.push({ mode, path: join(stateDir, `${mode}-state.json`), sessionId: null, ...legacy });
    ...
    try {
      ...
      continue; // no session-scoped layout for this state root
    }
    for (const sessionId of sessionIds) {
      if (excludeSessionId && sessionId === excludeSessionId) continue;
      const scoped = inspectStateFile(join(sessionsDir, sessionId, `${mode}-state.json`), now, thresholdMs);
      if (scoped) entries.push({ mode, path: join(sessionsDir, sessionId, `${mode}-state.json`), sessionId, ...scoped });
    }
  }
