...
const ANTI_SLOP_EXPLICIT_PATTERN =
  /\b(ai[\s-]?slop|anti[\s-]?slop|deslop|de[\s-]?slop)\b/i;
const ANTI_SLOP_ACTION_PATTERN =
  /\b(clean(?:\s*up)?|cleanup|refactor|simplify|dedupe|de-duplicate|prune)\b/i;
const ANTI_SLOP_SMELL_PATTERN =
  /\b(slop|duplicate(?:d|s)?|duplication|dead\s+code|unused\s+code|over[\s-]?abstract(?:ion|ed)?|wrapper\s+layers?|boundary\s+violations?|needless\s+abstractions?|unnecessary\s+abstractions?|ai[\s-]?generated|generated\s+code|tech\s+debt)\b/i;
...
const INFORMATIONAL_INTENT_PATTERNS = [
  /\b(?:what(?:'s|\s+is)|what\s+are|how\s+(?:to|do\s+i)\s+use|explain|explanation|tell\s+me\s+about|describe|difference\s+between|help\s+me\s+understand|what\s+does\s+\S+\s+do|can\s+you\s+explain)\b/i,
  /(?:뭐야|무엇(?:이야|인가요)?|어떻게|설명|사용법|알려줘|알려주세요|에\s*대해|가르쳐|소개|차이|비교|모르겠)/u,
  /(?:とは|って何|使い方|説明|教えて|について|知りたい|違い)/u,
  /(?:什么是|什麼是|怎(?:么|樣)用|如何使用|解释|說明|说明|告诉|告訴|关于|關於|介绍|介紹|区别|區別|了解)/u,
];
...
function activateState(directory, prompt, stateName, sessionId) {
  ...
  if (sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) {
    ...
    try {
      writeFileSync(
        join(sessionDir, `${stateName}-state.json`),
  ...
  try {
    writeFileSync(
      join(localDir, `${stateName}-state.json`),
      JSON.stringify(state, null, 2),
      { mode: 0o600 },
    );
  } catch {}
...
async function main() {
  ...
  const _skipHooks = (process.env.OMC_SKIP_HOOKS || "")
    .split(",")
    .map((s) => s.trim());
  if (
  ...
  try {
    ...
    if (
      hasActionableKeyword(
        cleanPrompt,
        /\b(autopilot|auto pilot|auto-pilot|autonomous|full auto|fullsend)\b/i,
      ) ||
      hasActionableKeyword(
        cleanPrompt,
    ...
    if (
      hasActionableKeyword(
        cleanPrompt,
        /\b(security\s+review|review\s+security)\b/i,
      )
    ) {
    ...
    if (
      hasActionableKeyword(
        cleanPrompt,
        /\b(ultrathink|think hard|think deeply)\b/i,
      )
    ) {
    ...
    if (
      ...
      hasActionableKeyword(
        cleanPrompt,
        /\bfind\s+(in\s+)?(codebase|code|all\s+files?)\b/i,
      )
    ) {
    ...
    // Analyze keywords
    if (
      hasActionableKeyword(cleanPrompt, /\b(deep[\s-]?analyze|deepanalyze)\b/i)
    ) {
    ...
    let tracer = null;
    try {
      tracer = await import("../dist/hooks/subagent-tracker/flow-tracer.js");
    } catch {
      /* silent */
    }
    ...
    if (followupPlanner && planningArtifacts) {
      ...
      const ralplanStatePath =
        sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)
          ? join(
              directory,
              ".omc",
              "state",
      ...
      if (ralplanWasActive) {
        ...
        const planningComplete =
          planningArtifacts.isPlanningComplete(artifacts);
        const context = { planningComplete, priorSkill: "ralplan" };

        const isTeamFollowup =
          followupPlanner.isApprovedExecutionFollowupShortcut(
            "team",
            prompt,
            context,
          );
        const isRalphFollowup =
          followupPlanner.isApprovedExecutionFollowupShortcut(
            "ralph",
            prompt,
            context,
        ...
        if (isTeamFollowup) {
          console.log(
            JSON.stringify(
              createHookOutput(createSkillInvocation("team", prompt)),
            ),
          );
          return;
        }
        if (isRalphFollowup) {
          console.log(
            JSON.stringify(
              createHookOutput(createSkillInvocation("ralph", prompt)),
            ),
          );
          return;
        }
    ...
    if (tracer) {
      for (const match of resolved) {
        try {
          tracer.recordKeywordDetected(directory, sessionId, match.name);
        } catch {
          /* silent */
        }
      }
    ...
    if (resolved.length > 0 && resolved[0].name === "cancel") {
      clearStateFiles(
        directory,
        ["ralph", "autopilot", "ultrawork", "swarm", "ralplan"],
        sessionId,
      );
      console.log(
        JSON.stringify(
          createHookOutput(createSkillInvocation("cancel", prompt)),
        ),
      );
      return;
    }
    ...
    if (tracer) {
      for (const mode of stateModes) {
        try {
          tracer.recordModeChange(directory, sessionId, "none", mode.name);
        } catch {
          /* silent */
        }
      }
    ...
    if (resolved.length === 0 && additionalContextParts.length > 0) {
      console.log(
        JSON.stringify(createHookOutput(additionalContextParts.join(""))),
      );
      return;
    }
