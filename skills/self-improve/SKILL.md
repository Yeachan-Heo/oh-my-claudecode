...
| Planning | Hypothesis → structured plan | lazycc:planner | opus |
| Architecture Review | 6-point plan review | lazycc:architect | opus |
| Critic Review | Harness rule enforcement | lazycc:critic | opus |
| Execution | Implement plan + run benchmark | lazycc:executor | opus |
| Git Operations | Atomic merge/tag/PR | lazycc:git-master | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `lazycc:git-master`:
  ```
...
Spawn N `lazycc:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `lazycc:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `lazycc:critic` with the plan + harness rules:
...
For each approved plan, spawn `lazycc:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `lazycc:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/lazycc:cancel` for clean state cleanup
