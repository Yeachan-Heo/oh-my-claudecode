...
3. **Architect** reviews for architectural soundness using `Task(subagent_type="oh-my-claudecode:architect", ...)`. Architect review **MUST** include: a first-principles "even better if" analysis (goal restatement stripped of implementation, 1-3 "even better if" optimizations generated from the goal, delta check of each against the plan), strongest steelman counterargument (antithesis) against the favored option informed by the "even better if" findings, at least one meaningful tradeoff tension, and (when possible) a synthesis path. In deliberate mode, Architect should explicitly flag principle violations. **Wait for this step to complete before proceeding to step 4.** Do NOT run steps 3 and 4 in parallel.
4. **Critic** evaluates against quality criteria using `Task(subagent_type="oh-my-claudecode:critic", ...)`. Critic **MUST** verify principle-option consistency, fair alternative exploration, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. Critic **MUST** verify the Architect performed a first-principles "even better if" analysis (goal restatement, ≥1 "even better if" optimization, delta check against the plan); if this analysis is missing or perfunctory (e.g., restating existing plan features as optimizations), **REJECT** and require Architect re-review. Critic **MUST** explicitly reject shallow alternatives, driver contradictions, vague risks, or weak verification. In deliberate mode, Critic **MUST** reject missing/weak pre-mortem or missing/weak expanded test plan. Run only after step 3 is complete.
...
- [ ] In consensus mode: Architect performed first-principles "even better if" analysis (goal restatement, ≥1 optimization, delta check) — not perfunctory
