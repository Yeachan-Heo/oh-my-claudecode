...
**C4 — decisions that emerge mid-run.** When a parallel Team worker hits a decision passing the ADR test, it stops before decision-dependent mutation and reports the question (options, recommendation, reversibility note) to the lead and `.omc/specs/<feature-slug>/decisions-pending.md`. With the worker's live claim token, use Team's supported `release-task-claim` operation to atomically return that task to `pending` and clear its owner/claim. This is a claim-release API operation, not a generic status transition and not a failure; do not mark the task `failed` or leave it `in_progress` while waiting on a human.

At the next batch boundary, assign each unresolved question a normal numeric Team decision task and pre-assign it to a configured Team worker (for example, the designated reviewer/decision worker). It follows only the supported lifecycle `pending` → `in_progress` → `completed`: that configured worker claims it, the lead presents the question batch to the human and records the answer in the decision log/ADR, then the worker completes the task from that recorded answer. The team lead never claims a task unless it is explicitly registered as a Team worker. Before re-dispatch, add the decision task ID to the released implementation task's `blockedBy`; Team's existing dependency resolution makes it eligible only after the decision task is completed. If a decision is visible before first dispatch, create this dependency pair immediately and keep the implementation task out of the frontier. Launch never requests a generic `in_progress` → `blocked` or `in_progress` → `pending` status transition.
