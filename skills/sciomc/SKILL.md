...
```
/lazycc:sciomc <goal>                    # Standard research with user checkpoints
/lazycc:sciomc AUTO: <goal>              # Fully autonomous until complete
/lazycc:sciomc status                    # Check current research session status
/lazycc:sciomc resume                    # Resume interrupted research session
/lazycc:sciomc list                      # List all research sessions
/lazycc:sciomc report <session-id>       # Generate report for session
```

### Quick Examples

```
/lazycc:sciomc What are the performance characteristics of different sorting algorithms?
/lazycc:sciomc AUTO: Analyze authentication patterns in this codebase
/lazycc:sciomc How does the error handling work across the API layer?
```
...
Task(subagent_type="lazycc:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="lazycc:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="lazycc:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="lazycc:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/lazycc:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/lazycc:sciomc AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="lazycc:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="lazycc:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="lazycc:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="lazycc:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="lazycc:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="lazycc:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="lazycc:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/lazycc:sciomc status` | Show current session progress |
| `/lazycc:sciomc resume` | Resume most recent interrupted session |
| `/lazycc:sciomc resume <session-id>` | Resume specific session |
| `/lazycc:sciomc list` | List all sessions with status |
| `/lazycc:sciomc report <session-id>` | Generate/regenerate report |
| `/lazycc:sciomc cancel` | Cancel current session (preserves state) |
