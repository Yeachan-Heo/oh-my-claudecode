...
| Command | Action |
|---------|--------|
| `/lazycc:writer-memory init <project-name>` | Initialize new project memory |
| `/lazycc:writer-memory status` | Show memory overview (character count, scene count, etc) |
| `/lazycc:writer-memory char add <name>` | Add new character |
| `/lazycc:writer-memory char <name>` | View character details |
| `/lazycc:writer-memory char update <name> <field> <value>` | Update character field |
| `/lazycc:writer-memory char list` | List all characters |
| `/lazycc:writer-memory rel add <char1> <char2> <type>` | Add relationship |
| `/lazycc:writer-memory rel <char1> <char2>` | View relationship |
| `/lazycc:writer-memory rel update <char1> <char2> <event>` | Add relationship event |
| `/lazycc:writer-memory scene add <title>` | Add new scene |
| `/lazycc:writer-memory scene <id>` | View scene details |
| `/lazycc:writer-memory scene list` | List all scenes |
| `/lazycc:writer-memory theme add <name>` | Add theme |
| `/lazycc:writer-memory world set <field> <value>` | Set world attribute |
| `/lazycc:writer-memory query <question>` | Query memory naturally (Korean supported) |
| `/lazycc:writer-memory validate <character> <dialogue>` | Check if dialogue matches character tone |
| `/lazycc:writer-memory synopsis` | Generate emotion-focused synopsis |
| `/lazycc:writer-memory export` | Export full memory as readable markdown |
| `/lazycc:writer-memory backup` | Create manual backup |
...
```
Task(subagent_type="lazycc:architect",
     model="opus",
