#!/usr/bin/env tsx
/**
 * discover.ts — Threads 제휴마케팅 채널 자동 발굴
 *
 * 키워드 검색 → 프로필 방문 → 팔로워/광고 필터 → 채널 목록 생성.
 * 이미 로그인된 Playwright Page를 받아 실행.
 *
 * Usage:
 *   npx tsx src/scraper/discover.ts --max 10
 *   npx tsx src/scraper/discover.ts --max 20 --keywords "쿠팡파트너스,핫딜"
 */
...
import { saveChannelsToDB } from './db-adapter.js';
...
const AD_PHRASES = [
  ...
  '링크 확인', '댓글 확인', '링크드려', '제가 써본',
  '써봤는데', '사용해봤', '추천드려', '추천합니다',
  '이거 진짜', '강추', '꿀팁',
];
...
/**
 * 팔로워 수 파싱 — collect.ts parseNum 패턴 참고
    ...
    const result = await page.evaluate(() => {
      ...
      if (!hasRecentPost) {
        ...
        const recentPatterns = [
          ...
          // Additional patterns for Threads Korean UI
          /\d+일/, /\d+주/, /\d+시간/, /\d+분/,
          // English compact formats
          /\d+h/, /\d+d/, /\d+w/, /\d+m/,
        ];
      ...
      // Fallback: if we found post links, the profile is active
      if (!hasRecentPost && postTexts.length > 0) {
        hasRecentPost = true;
      }
...
export async function discoverChannels(
  ...
  for (const keyword of keywords) {
    ...
    for (const channelId of candidateIds) {
      ...
      // Profile bio may contain ad signals — check bio text too
      const bioHasAdSignal = isAdText(profile.bio);
      const effectiveHasAd = hasAdPosts || bioHasAdSignal;

      // Consumer-need keywords don't require ad signals — active channels with content are valuable
      const CONSUMER_KEYWORDS = [
        '추천해줘', '뭐가좋아', '어디서사', '후기', '고민', '살까말까',
        '가성비', '꿀템', '인생템', '갓성비', '요즘뭐써', '이거써봤는데', '써본사람',
      ];
      const isConsumerKeyword = CONSUMER_KEYWORDS.includes(keyword);
      const hasEnoughContent = scan.totalScanned >= 3;

      // Ambiguous: followers 50~99, or has followers but no ad detection in posts
      const isAmbiguous = (
        (profile.follower_count >= 50 && profile.follower_count < 100) ||
        (scan.totalScanned > 0 && !effectiveHasAd && profile.follower_count >= 100)
      );

      if (hasEnoughFollowers && effectiveHasAd && isActive) {
        // Passed all filters
      ...
      } else if (isConsumerKeyword && hasEnoughFollowers && isActive && hasEnoughContent) {
        // Consumer-need keyword: accept active channels with enough content (no ad requirement)
        channels.push(discovered);
        excluded.add(channelId);
        passed++;
        log(`  ${channelId}: 선정-소비자 (팔로워 ${profile.follower_count}, 포스트 ${scan.totalScanned}, 키워드 "${keyword}")`);
  ...
  // DB dual-write
  try {
    const dbInserted = await saveChannelsToDB(channels);
    log(`DB 저장: ${dbInserted}/${channels.length}개 신규 채널`);
  } catch (e) {
    log(`DB 채널 저장 실패 (JSON은 정상 저장됨): ${(e as Error).message}`);
  }
...
const isDirectRun = process.argv[1] &&
  (process.argv[1].endsWith('discover.ts') ||
   process.argv[1].includes('discover'));
