...
| Ask providers                                  | `omc ask codex "review this patch"`           | `/ask codex "review this patch"`                                        | Both route through the same advisor flow. Providers: `claude`, `codex`, `gemini`, `grok`, `cursor`, `gjc`.                                            |
...
| **omc team (CLI)**          | tmux CLI workers — real `claude`/`codex`/`gemini`/`grok`/`cursor-agent`/`gjc` processes in split-panes       | Codex/Gemini/Grok/Cursor/GJC CLI tasks; on-demand spawn, die when done             |
...
omc ask gjc --prompt "apply this implementation plan"
...
/ask gjc "apply this implementation plan"
```
