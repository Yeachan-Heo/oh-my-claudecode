...
async function withDispatchLock(teamName, cwd, fn) {
  ...
  if (!existsSync3(root)) throw new Error(`Team ${teamName} not found`);
  ...
  await mkdir2(dirname2(lockDir), { recursive: true });
  while (true) {
    try {
...
async function writeDispatchRequestsToFile(teamName, requests, cwd) {
  ...
  await writeFile2(tmpPath, JSON.stringify(requests, null, 2), "utf-8");
  const { rename: rename3 } = await import("fs/promises");
  await rename3(tmpPath, path4);
}
...
import { join as join5, basename, isAbsolute as isAbsolute2, win32 } from "path";
...
function resolveShellFromCandidates(paths, rcFile) {
  for (const p of paths) {
    if (existsSync4(p)) return { shell: p, rcFile };
  }
  return null;
}
function resolveSupportedShellAffinity(shellPath) {
  ...
  if (!existsSync4(shellPath)) return null;
...
import { join as join6, dirname as dirname3, basename as basename2, resolve, relative, isAbsolute as isAbsolute3 } from "path";
import { fileURLToPath } from "url";
function getPackageDir() {
  if (typeof __dirname !== "undefined" && __dirname) {
    const currentDirName = basename2(__dirname);
    const parentDirName = basename2(dirname3(__dirname));
...
import { join as join7, dirname as dirname4, basename as basename3 } from "path";
...
import { writeFileSync, existsSync as existsSync7, mkdirSync, renameSync, openSync, writeSync, closeSync, realpathSync, constants } from "fs";
import { dirname as dirname7, resolve as resolve2, relative as relative2, basename as basename4 } from "path";
function atomicWriteJson(filePath, data, mode = 384) {
  const dir = dirname7(filePath);
  if (!existsSync7(dir)) mkdirSync(dir, { recursive: true, mode: 448 });
  const tmpPath = `${filePath}.tmp.${process.pid}.${Date.now()}`;
  writeFileSync(tmpPath, JSON.stringify(data, null, 2) + "\n", { encoding: "utf-8", mode });
  renameSync(tmpPath, filePath);
}
function ensureDirWithMode(dirPath, mode = 448) {
  if (!existsSync7(dirPath)) mkdirSync(dirPath, { recursive: true, mode });
}
function safeRealpath(p) {
  try {
    ...
    try {
      return resolve2(realpathSync(parent), name);
    } catch {
      return resolve2(p);
    }
  }
...
async function requeueDeadWorkerTasks(teamName, deadWorkerNames, cwd) {
  ...
  for (const task of tasks) {
    ...
    try {
      const { withFileLock: withFileLock2 } = await Promise.resolve().then(() => (init_file_lock(), file_lock_exports));
      const { readFile: readFileAsync, writeFile: writeFileAsync, rename: renameAsync } = await import("fs/promises");
      await withFileLock2(taskPath2 + ".lock", async () => {
        const raw = await readFileAsync(taskPath2, "utf-8");
        const taskData = JSON.parse(raw);
        if (taskData.status === "in_progress") {
          taskData.status = "pending";
          taskData.owner = void 0;
          taskData.claim = void 0;
          const tmpPath = taskPath2 + ".tmp." + process.pid;
          await writeFileAsync(tmpPath, JSON.stringify(taskData, null, 2), "utf-8");
          await renameAsync(tmpPath, taskPath2);
...
function resolveJobsDir(env = process.env) {
  return env.OMC_JOBS_DIR || join17(homedir2(), ".omc", "team-jobs");
}
