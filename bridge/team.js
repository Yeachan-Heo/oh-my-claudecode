...
function processGroupIdSync(pid) {
  ...
  if (process.platform === "linux") {
    try {
      ...
      return linuxProcRecordFromStat(stat2)?.processGroupId ?? null;
    } catch {
      return null;
    }
  }
...
function linuxProcStateFromStat(stat2) {
  return linuxProcRecordFromStat(stat2)?.state ?? null;
}
function linuxProcRecordFromStat(stat2) {
...
function readLinuxProcState(pid) {
  try {
    return linuxProcStateFromStat(readFileSync4(`/proc/${pid}/stat`, "utf8"));
  } catch {
    return null;
  }
}
function isProcessGroupQuiescent(processGroupId) {
  ...
  if (process.platform === "linux") {
    try {
      for (const entry of readdirSync3("/proc")) {
        ...
        let stat2;
        try {
          stat2 = readFileSync4(`/proc/${entry}/stat`, "utf8");
        } catch (error) {
          if (error.code === "ENOENT") continue;
          return false;
        }
        const record = linuxProcRecordFromStat(stat2);
...
async function getProcessStartTimeLinux(pid, deadlineAt) {
  ...
  try {
    ...
    return linuxProcRecordFromStat(stat2)?.startTime ?? void 0;
  } catch {
    return void 0;
  }
}
function getProcessStartIdentitySync(pid) {
  ...
  if (process.platform === "linux") {
    try {
      const stat2 = readFileSync4(`/proc/${pid}/stat`, "utf8");
      const startTime = linuxProcRecordFromStat(stat2)?.startTime;
      return startTime === null || startTime === void 0 ? null : String(startTime);
    } catch {
      return null;
    }
  }
...
function getValidAgentRoles() {
  ...
  try {
    ...
    const files = readdirSync4(agentsDir);
...
import { readdirSync as readdirSync5, readFileSync as readFileSync8, unlinkSync as unlinkSync6 } from "fs";
...
import { existsSync as existsSync12, readFileSync as readFileSync9, readdirSync as readdirSync6, statSync as statSync3, lstatSync, unlinkSync as unlinkSync7, rmSync, renameSync as renameSync3, symlinkSync } from "fs";
...
import { existsSync as existsSync15, readdirSync as readdirSync8, readFileSync as readFileSync11 } from "fs";
...
import { existsSync as existsSync19, realpathSync as realpathSync5, readFileSync as readFileSync14, readdirSync as readdirSync9, rmSync as rmSync2, unlinkSync as unlinkSync8, writeFileSync as writeFileSync5 } from "node:fs";
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  ...
  const issues = [];
  for (const workerName of readdirSync9(workersDir)) {
...
import { existsSync as existsSync27, linkSync as linkSync3, mkdirSync as mkdirSync7, readFileSync as readFileSync17, readdirSync as readdirSync11, renameSync as renameSync4, unlinkSync as unlinkSync10, writeFileSync as writeFileSync6 } from "fs";
...
function resolveTaskAssignment(task, resolvedRouting, roleRoutingConfig, fallbackAgent, providerExplicit = fallbackAgent !== "claude") {
...
async function startTeamV2(config) {
  ...
  const selectedProviders = /* @__PURE__ */ new Set();
  const effectiveAssignments = /* @__PURE__ */ new Map();
  for (let i = 0; i < workerNames.length; i++) {
    ...
    const assignment = taskIndex === void 0 ? { agentType: fallbackAgent, model: "", role: void 0 } : resolveTaskAssignment(
  ...
  for (let i = 0; i < workerNames.length; i++) {
    ...
    const workerCwd = worktreeMode === "disabled" ? leaderCwd : getWorktreePath(leaderCwd, sanitized, workerName);
...
init_platform();
init_state_paths();
import { readFileSync as readFileSync15, readdirSync as readdirSync10, existsSync as existsSync20, openSync as openSync4, closeSync as closeSync4, unlinkSync as unlinkSync9, writeSync as writeSync4, statSync as statSync4, constants as fsConstants2 } from "fs";
...
init_worktree_paths();
import { readdirSync as readdirSync13, readFileSync as readFileSync20, existsSync as existsSync31 } from "fs";
