...
async function killProcessTreeWindows(pid, force) {
  try {
    ...
    (0, import_child_process4.execFileSync)("taskkill.exe", args, {
      stdio: "ignore",
      timeout: 5e3,
      windowsHide: true
    });
...
${waitsForBatchProvider ? 'start "" /wait /b ' : ""}${renderedProvider}\r
`
    };
  }
  return { command, args };
...
async function materializeProviderSpawnInvocation(invocation, options = {}) {
  ...
  const completionPath = options.superviseWindowsTree ? (0, import_node_path.join)(wrapperDir, "provider-exit.txt") : void 0;
