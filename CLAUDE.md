...
Lifecycle: `context-doctor` (context audit/optimize), `cost-report` (token cost & model routing), `ci-pipeline` (CI/CD generation/debugging), `security-audit` (threat model & secret scanning), `onboard` (codebase onboarding & architecture tour), `prompt-library` (prompt templates), `perf-audit` (performance profiling & benchmarks), `meta-debug` (hallucination/loop/confusion detection), `team-conventions` (team CLAUDE.md & governance), `data-analyze` (EDA/reports/log analysis), `hooks-guide` (hook authoring & debugging), `workflow-automation` (webhooks/n8n/scheduled pipelines), `think-plan` (extended thinking & plan mode guide), `claude-md-templates` (project-type CLAUDE.md templates), `visual-context` (screenshot/mockup/diagram input), `headless-mode` (scripted/batch/SDK automation)
