...
Per-role `/team` routing: configure provider/model per canonical role (codex critic, gemini reviewer, etc.) in `.claude/omc.jsonc` under `team.roleRouting` — see `skills/team/SKILL.md#per-role-provider--model-routing`.
</skills>
